...
<center> 
   <form name="seebookdetails" action="./AddItemOrdrBookAction" method="">     
<table align="center" WIDTH="650" border="2" bordercolor="black" bgcolor="#F4F5F7">
      <th colspan="15" bgcolor="lightgreen"><center><b><font color="white">BOOKING  DETAILS</font></center></th>
      <tr rowspan="4"></tr>
    ...
    <%if(request.getParameter("status")!=null)out.print(request.getParameter("status"));    %> </span> 
                ...
                <%
                 BookTypeModel  em=null;
                String images=getServletContext().getRealPath("/userimages");
                int roomtypeid=Integer.parseInt(request.getParameter("bktypeid").trim());
                String loginname=(String)session.getAttribute("user"); 

       try{
      
       em=new BookTypeDAO().getBookingTypesDetails(roomtypeid);
         
          
         %> 
                  
        <tr>
    <input type="hidden" name="bktid" value="<%=em.getBooktypeid()%>"/>
     <td>Book Type Name </td>
    <td><input type="text"  name="bktname"  value="<%=em.getBooktypename()%>" readonly /></td>
     </tr>
     
      <tr>
    <td>Cost /Hour</td>
    <td><input type="text" name="cost" value="<%=em.getCostperHour()%>"  /></td>
    </tr>
          
      <tr>
    <td>Descrition</td>
    <td><input type="text" name="desc" value="<%=em.getBooktypedesc()%>" /></td>
    </tr>
     
     <tr>
    <td>Status</td>
    <td><input type="text" name="status" value="<%=em.getBooktypestatus()%>"/></td>
    </tr>
...
{e.printStackTrace();}%> 
  ...
  <%if(role.equalsIgnoreCase("Admin")){ %>
      <input type="submit" name="submit" value="Update Book Type Details"/>
