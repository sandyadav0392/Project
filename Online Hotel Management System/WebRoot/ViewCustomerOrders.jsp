...
<table align="center" WIDTH="630" border="0" bordercolor="black" bgcolor="#F4F5F7">
      <th colspan="15" bgcolor="lightgreen"><center><b><font color="white">View All Orders</font></center></th>
      <tr rowspan="4"></tr>
  ...
  OrderModel  osmodel=null;

       try{
       
        String details=getServletContext().getRealPath("/Doc");
       CoreHash ch=new ItemDAO().getCostOrderShips();
     ...
     <td>State</td>    
        ...
        <% while(em.hasMoreElements()){
             osmodel=(OrderModel)em.nextElement();
             
            
         %> 
        <tr> 
        <td value="<%=osmodel.getOrderid() %>"><a href="SeeCustOrdersByEmp.jsp?orderid=<%=osmodel.getOrderid() %>&orderid=<%=osmodel.getOrderid()%>"><%=osmodel.getOrderid() %> </a></td>
        <td ><%=osmodel.getOrdertypename() %></td>
        <td value="<%=osmodel.getCustid() %>"><%=osmodel.getCustname() %></td>
         <td ><%=osmodel.getShipingid() %></td>
        <td value="<%=osmodel.getItemid() %>"><%=osmodel.getItemname() %></td>       
        <td value="<%=osmodel.getHotelid() %>"><%=osmodel.getHotelname() %></td>        
        <td ><%=osmodel.getQuantity() %></td>
        <td ><%=osmodel.getOrderdate() %></td>
        <td ><%=osmodel.getOrderstate() %></td>             
          </tr>
