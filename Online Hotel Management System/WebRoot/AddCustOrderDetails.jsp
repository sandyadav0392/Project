...
<%@page import="com.dts.project.model.ItemModel"%>
...
<%@page import="com.dts.project.dao.CustomerDAO"%><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
function inDate(){
  ...
  var date=new Date();
  document.custorder.stdate.value=date.getDate()+"-"+(date.getMonth()+1)+"-"+date.getYear();
...
function TotalCost(){
	...
	var ocost1=document.custorder.itemcost.value;
	alert(ocost1);
	var scost1=document.custorder.quantity.value;	
alert(scost1);
	document.custorder.tcost.value=parseInt(ocost1)*parseInt(scost1);
alert(tcost);
}
...
function goOn() {
alert("hello");
var port=document.custorder.port.value;
alert(port);
var host=document.custorder.host.value;
alert(host);
var iid=document.custorder.itemid.value;
alert(iid);
window.location.href='http://'+host+':'+port+'/HUNGRY1/GetItemCostAction?itemid='+iid;
...
<SCRIPT language="JavaScript" type="text/javascript" src="images/ScriptServlet">
</SCRIPT>
...
<center>
 <form method="" action="./AddItemOrdrBookAction" name="custorder" > 
     ...
     <center>  <% if(request.getParameter("status")!=null)
            ...
            String userimage=getServletContext().getRealPath("/userimages");
           
            CustomerModel cmodel=new CustomerDAO().getCustProfile(loginname);
             int cid=cmodel.getCustid();
                   
                   
                    ItemModel  im=(ItemModel)session.getAttribute("custordritm");
 ...
 <tr><input type="hidden" name="itemid"    value="<%=im.getItemid()%>"  ><td> Order Item</td><td><input type="text" name="itemname"  onblur="goOn()" value="<%=im.getItemname() %>"></td></tr>
 <tr><td> Order Type</td><td><select name="otype">
 <option value="Home">Home</option>
 </select></td></tr> 

 <tr><td> Order Quantity</td><td><input type="text" name="quantity"></td></tr>
 <!-- <tr><td> Order Cost</td><td><input type="text" name="cost"></td></tr>  --> 
 <tr><td> Order Date</td><td><input type="text" name="stdate" onfocus="inDate()" ></td></tr>
 
 <tr><td> Item Cost</td><td><input type="text" name="itemcost"  value="<%if(request.getParameter("icost")!=null) out.println(request.getParameter("icost")); else out.println(""); %>" /></td></tr>
...
<% 
CoreHash ch1=new HotelDAO().getHotelNames();
	             Enumeration em1=ch1.elements();	            
	             HotelModel p1=null;
	             while(em1.hasMoreElements()){
	                   p1=(HotelModel)em1.nextElement();	                    
	                   %>                    
                   <option  value="<%=p1.getHotelid()%>"><%=p1.getHotelname()%></option> 
                   <% }   %>
</select>
    </td>
    </tr> 
    
    <tr><input type="hidden" name="custid" value="<%=cid%>"></tr>
   <tr><td>ToTal Cost</td><td><input type="text" name="tcost" onfocus="TotalCost()"></td></tr> 
    
   <tr><td> Card Number</td><td><input type="text" name="cnum" value="" ></td></tr> 
   <tr><td> PIN Number</td><td><input type="password" name="pnum" value="" ></td></tr> 
   <tr><td> EXP Date</td><td><input type="text" name="exdate" value="" ></td></tr> 
   <tr><td> CVV2</td><td><input type="text" name="cvv" value="" ></td></tr> 
  
  
   <tr><td><input type="submit" name="submit" value="Give Order"></td>
<%--   <td><input type="submit" name="submit" value="Make New Order"></td>--%>
