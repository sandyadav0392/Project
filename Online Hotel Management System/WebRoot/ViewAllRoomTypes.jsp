...
<%@page import="com.dts.project.dao.RoomDAO"%><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
function inDate(){
  ...
  var date=new Date();
  document.addhotel.stdate.value=date.getDate()+"-"+(date.getMonth()+1)+"-"+date.getYear();
...
<center>
    ...
    <form name="" action="" method="post">    
		 <span class="style13">Export to : <a href="ExportXLS" target="_blank"><font color="brown">XLS</font></a></span><br/>
            <br/> 
<table align="center" WIDTH="630" border="0" bordercolor="black" bgcolor="#F4F5F7">
      <th colspan="15" bgcolor="lightgreen"><center><b><font color="white">View All Room Types</font></center></th>
      <tr rowspan="4"></tr>
  ...
  RoomTypeModel  rmodel=null;

       try{
       
        String details=getServletContext().getRealPath("/userimages");
       CoreHash ch=new RoomDAO().getAllRoomTypesDetails();
        Enumeration em=ch.elements();
         String report="";
            report+=" <table align='center' WIDTH='630' border='0' bordercolor='black' bgcolor='#F4F5F7'>";
         report+="<th colspan='15' bgcolor='lightgreen'><center><b><font color='white'>View All Room Types</font></center></th>";
      report+="<tr rowspan='4'></tr>";
        ...
        if(request.getParameter("status")!=null)
           ...
           report+="<strong> <table align='center' WIDTH='700' border='0' bordercolor='block' bgcolor=''> <tr>    <td>Room Type Name</td>     <td>Cost/Room</td>     <td>No.Of Rooms</td>   <td>Hotel Name</td>    <td>Facility Type</td>     <td>Room Description</td> <td>Room Status</td>     </tr>     </table>  </strong>       <table align='center' WIDTH='750' border='0' bordercolor='black' bgcolor=''>";  
         
     
         %> 
         <strong>
 <table align="center" WIDTH="700" border="0" bordercolor="block" bgcolor=""> 
           <tr>
    ...
    <td>Room Status</td>
       ...
       <table align="center" WIDTH="750" border="0" bordercolor="black" bgcolor="">
        <% 
        
        while(em.hasMoreElements()){
             rmodel=(RoomTypeModel)em.nextElement();
           
         
         report+=" <tr>";
 report+="<td value="+rmodel.getRoomtypeid() +">"+rmodel.getRoomtypename()+"</td>";
  report+="<td>"+rmodel.getCost4room()+"</td>";
   report+="<td>"+rmodel.getNoofroom()+"</td>";
   report+="<td value="+rmodel.getHotelid()+">"+rmodel.getHotelname()+"</td>";
   report+="<td value="+rmodel.getFacilityid()+">"+rmodel.getFacilityname()+"</td>";
   report+="<td>"+rmodel.getRoomdesc()+"</td>";
   report+="<td>"+rmodel.getRoomstatus()+"</td> ";  
  report+="</tr> "; 
      session.setAttribute("Report",report);      
        ...
        <tr>
 <td value="<%=rmodel.getRoomtypeid() %>"><a href="SeeRoomTypeDetails.jsp?roomtypeid=<%=rmodel.getRoomtypeid()%>"><%=rmodel.getRoomtypename() %></a></td>
  <td ><%=rmodel.getCost4room() %></td>
   <td ><%=rmodel.getNoofroom() %></td>
   <td value="<%=rmodel.getHotelid() %>"><%=rmodel.getHotelname() %></td>
   <td value="<%=rmodel.getFacilityid() %>"><%=rmodel.getFacilityname() %></td>
   <td ><%=rmodel.getRoomdesc() %></td>
   <td ><%=rmodel.getRoomstatus() %></td>   
  </tr>
  <%} %>
  </table>
