...
<%@page import="com.dts.project.dao.FacilityDAO"%>
...
<center> 
   <form name="seehoteldetails" action="./AddItemOrdrBookAction" method="">     
<table align="center" WIDTH="650" border="0" bordercolor="black" bgcolor="#F4F5F7">
    ...
    <%if(request.getParameter("status")!=null)out.print(request.getParameter("status"));    %> </span> 
       ...
       em=new RoomDAO().getPertRoomTypesDetails(roomtypeid);
         
          
         %> 
                  
        <tr>
    <input type="hidden" name="rtid" value="<%=em.getRoomtypeid()%>"/>
     <td>Room Type Name </td>
    <td><input type="text"  name="rtname"  value="<%=em.getRoomtypename()%>" readonly /></td>
     </tr>
     
      <tr>
    <td>Cost For Room</td>
    <td><input type="text" name="cost" value="<%=em.getCost4room()%>"  /></td>
    </tr>
          
      <tr>
    <td>No Of Rooms</td>
    <td><input type="text" name="noofroom" value="<%=em.getNoofroom()%>" /></td>
    </tr>
     
<%--     <tr><input type="hidden" name="hotelid" value="<%=em.getHotelid()%>"/>--%>
<%--    <td>Hotel Name</td>--%>
<%--    <td><input type="text" name="hotelname" value="<%=em.getHotelname()%>"/></td>--%>
<%--    </tr>--%>
...
CoreHash ch1=new HotelDAO().getHotelNames();
	             ...
	             while(em1.hasMoreElements()){
	                   p2=(HotelModel)em1.nextElement();
	                     if(em.getHotelid()==p2.getHotelid()){
	                   %>
                    <option selected value="<%=em.getHotelid()%>"><%=em.getHotelname()%></option>
                    <%} else %>
                   <option  value="<%=p2.getHotelid()%>"><%=p2.getHotelname()%></option> 
                   <% }%>
</select>
    </td>
    </tr>    
...
<%--   <tr><input type="hidden" name="faciltyid" value="<%=em.getFacilityid()%>"/>--%>
<%--    <td>Facility Name</td>--%>
<%--    <td><input type="text" name="state" value="<%=em.getFacilityname()%>"/></td>--%>
<%--    </tr>--%>
    
    <tr>
    <td>Facility Name</td>
    <td><select  name="facilname">
<%--<option value="select">-Select-</option>--%>
<% 
CoreHash ch2=new FacilityDAO().getFacilityTypes();
	             Enumeration em2=ch2.elements();
	            
	             FacilityModel f2=null;
	             while(em2.hasMoreElements()){
	                   f2=(FacilityModel)em2.nextElement();
	                     if(em.getFacilityid()==f2.getFacilid()){
	                   %>
                    <option selected value="<%=em.getFacilityid()%>"><%=em.getFacilityname()%></option>
                    <%} else %>
                   <option  value="<%=f2.getFacilid()%>"><%=f2.getFacilname()%></option> 
                   <% }%>
</select>
    </td>
    </tr>    
     ...
     <tr>
    <td>Room Desc</td>
    <td><input type="text" name="rdesc"   value="<%=em.getRoomdesc()%>"/></td>
    </tr>
    
    
     <tr>
    <td>Room Status</td>
    <td><input type="text" name="rstatus" value="<%=em.getRoomstatus()%>"/></td>
    </tr>
...
{e.printStackTrace();}%> 
         ...
         <tr rowspan="1"></tr><tr rowspan="1"></tr>
         <tr rowspan="1"></tr><tr rowspan="1"></tr>
      <tr><td colspan="2"><center>
  <%if(role.equalsIgnoreCase("Admin")){ %>
      <input type="submit" name="submit" value="Update Room Type Details"/>
