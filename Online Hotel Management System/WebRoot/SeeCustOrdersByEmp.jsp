...
<%@page import="com.dts.project.dao.ItemDAO"%>
<%@page import="com.dts.project.model.OrderModel"%>
...
function inDate(){
  ...
  var date=new Date();
  document.seecustordrs.shippeddate.value=date.getDate()+"-"+(date.getMonth()+1)+"-"+date.getYear();
...
<center> 
   <form name="seecustordrs" action="./AddItemOrdrBookAction" method="">     
<table align="center" WIDTH="650" border="0" bordercolor="black" bgcolor="#F4F5F7">
      <th colspan="15" bgcolor="lightgreen"><center><b><font color="white">CUSTOMER  ORDER DETAILS</font></center></th>
      <tr rowspan="4"></tr>
    ...
    <%if(request.getParameter("status")!=null)out.print(request.getParameter("status"));    %> </span> 
                ...
                <%
                 ...
                 EmpModel emodel=null;
                String docu=getServletContext().getRealPath("/docu");
                int oid=Integer.parseInt(request.getParameter("orderid").trim());
                String loginname=(String)session.getAttribute("user"); 
                emodel=new EmpRegistrationDAO().getEmployeesPersonalProfiles(docu,loginname);
int empid=emodel.getEmpid();
       try{
      
       omodel=new ItemDAO().getEachCustOrderShipsDetails(oid);
     ...
     <td>OrDer ID </td>
    <td><input type="text" name="ordrid" readonly value="<%=omodel.getOrderid()%>"/></td>
     </tr>
        
        <tr>
   
     <td>Order Type </td>
    <td><input type="text"  name="otype"  readonly value="<%=omodel.getOrdertypename()%>" readonly /></td>
     </tr>
     <input type="hidden" name="empid" value="<%=empid%>"  />
      <tr><input type="hidden" name="custid" value="<%=omodel.getCustid()%>"  />
    <td>Cust Name</td>
    <td><input type="text" name="cname" readonly value="<%=omodel.getCustname()%>"  /></td>
    </tr>
     ...
     <td>Shipping ID </td>
    <td><input type="text" name="shpid" readonly value="<%=omodel.getShipingid()%>"/></td>
     </tr>
        
        <tr><input type="hidden" name="htlid" value="<%=omodel.getHotelid()%>"  />
    <td>Hotel Name</td>
    <td><input type="text" name="htlname" readonly value="<%=omodel.getHotelname()%>"  /></td>
    </tr>
     
     <tr>
    <td>Cost</td>
    <td><input type="text" name="cost" readonly value="<%=omodel.getTotalcost()%>"/></td>
    </tr>
   <tr>
    <td>Cart Number</td>
    <td><input type="text" name="cnum" readonly value="<%=omodel.getCartid()%>"/></td>
    </tr>
    
    
     <tr>
    <td>Quantity</td>
    <td><input type="text" name="quantity"  readonly value="<%=omodel.getQuantity()%>"/></td>
    </tr>
    
    
     <tr>
    <td>Order Date</td>
    <td><input type="text" name="odate" readonly value="<%=omodel.getOrderdate()%>"/></td>
    </tr>
    
    <tr>
    <td>Shipped Date</td>
    <td><input type="text" name="shippeddate"  value="" onfocus="inDate()"/></td>
    </tr>
    ...
    <% if(omodel.getOrderstate().equalsIgnoreCase("ACTIVE")){ %>
...
{e.printStackTrace();}%> 
  ...
  <%if(role.equalsIgnoreCase("Admin")||role.equalsIgnoreCase("employee")){ %>
      <input type="submit" name="submit" value="Update Cust Ordr Shiping Details"/>
